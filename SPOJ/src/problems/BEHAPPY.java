...
public class BEHAPPY {
    ...
    private List<List<Integer>> calculate(List<List<Integer>> input, int M, int N) {
        ...
        if (input.isEmpty()) { // if no more elements to process
            ...
            process(input, res, M, N); // we need to calculate the cartesian product of input and store it in res
                                       // variable
        }
    ...
    private void process(List<List<Integer>> lists, List<List<Integer>> res, int M, int N) {
        ...
        List<List<Integer>> tail = calculate(lists.subList(1, lists.size()), M, N); // invoke calculate on remaining
                                                                                    // element, here is recursion
